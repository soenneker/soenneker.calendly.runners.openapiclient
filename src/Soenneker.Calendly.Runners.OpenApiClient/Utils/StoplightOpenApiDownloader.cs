...
/// <summary>
/// Bundles a modular Stoplight OpenAPI document into a single YAML file by inlining external relative refs.
/// </summary>
public sealed class StoplightOpenApiBundler
{
    private const string StoplightApiBase = "https://stoplight.io/api/v1/projects/";
    ...
    public async ValueTask<string> BundleAsync(string projectId, string rootNodePath, string outputFilePath, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Starting Stoplight OpenAPI bundle for project '{ProjectId}' from '{RootNodePath}' to '{OutputFilePath}'",
            projectId, normalizedRootNodePath, fullOutputFilePath);
        ...
        YamlNode rootNode = await GetOrParseYamlRootAsync(projectId, normalizedRootNodePath, cache, cancellationToken).ConfigureAwait(false);
        YamlNode bundledRootNode = await ResolveExternalRefsAsync(rootNode, projectId, normalizedRootNodePath, cache, resolutionStack, cancellationToken).ConfigureAwait(false);
    ...
    /// <param name="cancellationToken">Cancellation token.</param>
    /// <returns>The full local path to the bundled file.</returns>
    public ValueTask<string> BundleAsync(string stoplightNodeUrl, string outputFilePath, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Parsed Stoplight bundle URL '{StoplightNodeUrl}' to project '{ProjectId}' and node '{NodePath}'",
            stoplightNodeUrl, projectId, nodePath);

        return BundleAsync(projectId, nodePath, outputFilePath, cancellationToken);
    }
    ...
    {
        string requestUri = $"{StoplightApiBase}{Uri.EscapeDataString(projectId)}/api-docs/nodes/{EscapeNodePathForUrl(nodePath)}";
    ...
    private static IReadOnlyList<string> ExtractExternalRefs(string content)
    {
        ...
        return LooksLikeJson(trimmed)
            ? ExtractRefsFromJson(content)
            : ExtractRefsFromYaml(content);
    }
                ...
                case YamlMappingNode mapping:
                    {
                        foreach ((YamlNode keyNode, YamlNode valueNode) in mapping.Children)
                        {
                            if (keyNode is YamlScalarNode keyScalar &&
                                ...
                                !string.IsNullOrWhiteSpace(valueScalar.Value))
                            {
    ...
    {
        string path = nodePath.Replace('\\', '/').Trim();
    ...
    {
        return string.Join('/', nodePath.Split('/').Select(Uri.EscapeDataString));
    }
        ...
        ISet<string> resolutionStack, CancellationToken cancellationToken)
    {
        switch (node)
        {
            ...
            {
                if (TryGetRefValue(mappingNode, out string? reference) &&
                    !string.IsNullOrWhiteSpace(reference) &&
                    !IsLocalDocumentRef(reference) &&
                    !IsRemoteAbsoluteUrl(reference))
                {
                    ...
                    {
                        _logger.LogError("Circular external reference detected while bundling project '{ProjectId}': current '{CurrentNodePath}', ref '{Reference}'",
                    ...
                    try
                    {
                        YamlNode resolvedRefNode = await ResolveReferencedNodeAsync(reference, projectId, currentNodePath, cache, resolutionStack, cancellationToken).ConfigureAwait(false);
                        ...
                        foreach ((YamlNode key, YamlNode value) in mappingNode.Children)
                        {
                            ...
                            mergedMapping.Children[CloneYamlNode(key)] = await ResolveExternalRefsAsync(value, projectId, currentNodePath, cache, resolutionStack, cancellationToken)
                ...
                foreach ((YamlNode key, YamlNode value) in mappingNode.Children)
                {
                    resolvedMapping.Add(CloneYamlNode(key), await ResolveExternalRefsAsync(value, projectId, currentNodePath, cache, resolutionStack, cancellationToken)
                        .ConfigureAwait(false));
                }
            ...
            case YamlSequenceNode sequenceNode:
            {
                ...
                {
                    resolvedSequence.Add(await ResolveExternalRefsAsync(child, projectId, currentNodePath, cache, resolutionStack, cancellationToken).ConfigureAwait(false));
                }
    ...
    private async ValueTask<YamlNode> ResolveReferencedNodeAsync(string reference, string projectId, string currentNodePath, IDictionary<string, YamlNode> cache,
        ISet<string> resolutionStack, CancellationToken cancellationToken)
    {
        ...
        _logger.LogDebug("Resolved external ref '{Reference}' from '{CurrentNodePath}' to node '{ReferencedNodePath}' with fragment '{Fragment}'",
            reference, currentNodePath, referencedNodePath, fragment ?? "<none>");
        YamlNode referencedRootNode = await GetOrParseYamlRootAsync(projectId, referencedNodePath, cache, cancellationToken).ConfigureAwait(false);
        YamlNode targetNode = string.IsNullOrWhiteSpace(fragment)
            ? referencedRootNode
            : ResolveJsonPointer(referencedRootNode, fragment!, referencedNodePath);

        return await ResolveExternalRefsAsync(targetNode, projectId, referencedNodePath, cache, resolutionStack, cancellationToken).ConfigureAwait(false);
    }

    private async ValueTask<YamlNode> GetOrParseYamlRootAsync(string projectId, string nodePath, IDictionary<string, YamlNode> cache, CancellationToken cancellationToken)
    {
        ...
        string yaml = await FetchNodeContentAsync(projectId, nodePath, cancellationToken).ConfigureAwait(false);
    ...
    private static YamlNode ResolveJsonPointer(YamlNode rootNode, string fragment, string filePath)
    {
        ...
        string[] segments = fragment[2..].Split('/', StringSplitOptions.None)
